...
impl<A: AppAssetId> Renderer<A> {
    pub(crate) fn new(info: &AppInfo, sprites_tex: Texture, tiles_tex: Texture) -> Renderer<A> {
        Renderer {
            r: core::Renderer::new(info, sprites_tex, tiles_tex),
            phantom: PhantomData,
        }
    }
...
impl<'a, A: AppAssetId + 'a> SpriteRenderer<'a, A> {
    ...
    pub fn draw_flash(&mut self, affine: &Affine, sprite: A::Sprite, flash_ratio: f64) {
        self.r.r.draw_sprite(affine, sprite.id_u16(), flash_ratio);
    }
}
...
impl<'a, A: AppAssetId + 'a> TiledRenderer<'a, A> {
    ...
    pub fn draw(&mut self, affine: &Affine, tile: A::Tile) {
        self.r.r.draw_tile(self.camera, affine, tile.id_u16());
    }
}
